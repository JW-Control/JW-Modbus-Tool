...
const active = () => {
  const activeButton = Array.from(document.querySelectorAll(".sidebar button.active")).some((button) => button.textContent?.includes("Pruebas"));
...
function mk(slave, fn, addr, amount, exp, to = 1000) { return { on: true, slave, fn, addr, amount: String(amount), exp, to, res: "Pendiente", ms: null, detail: "", values: "", at: "" }; }
function cloneStep(x) { return { ...x, res: "Pendiente", ms: null, detail: "", values: "", at: "" }; }
function cleanStep(x) {
  const fn = F[x?.fn] ? x.fn : "fc3";
  return { on: x?.on !== false, slave: Math.max(1, Math.min(247, Number.parseInt(String(x?.slave ?? 1), 10) || 1)), fn, addr: Number.isFinite(Number(x?.addr)) ? Number(x.addr) : def(fn), amount: String(x?.amount ?? defaultAmount(fn)), exp: String(x?.exp ?? defaultExpected(fn, x?.amount ?? defaultAmount(fn))), to: Number.isFinite(Number(x?.to)) ? Number(x.to) : 1000, res: "Pendiente", ms: null, detail: "", values: "", at: "" };
}
function plan(slave) {
  return [
...
function res(r) { const c = r === "Aprobado" ? "resultOK" : r === "Pendiente" ? "" : r === "Timeout" ? "resultWarn" : "resultBad"; const icon = r === "Aprobado" ? "✓" : r === "Pendiente" ? "—" : r === "Timeout" ? "!" : "×"; return `<span class="${c} resultPill">${icon} ${r}</span>`; }
function msg(t) { const p = document.querySelector(".helper p"); if (p) p.textContent = t; }

function exportRuntimeState() { return { format: runtimeFormat, version: 1, savedAt: new Date().toISOString(), selectedScenario: st.sc, simulatorState: st.sim, steps: st.steps.map(cleanStep), scenarios: JSON.parse(JSON.stringify(st.scenarios)) }; }
...
function exposeRuntimeState() {
  window.__jwSimpleTestsRuntimeState = { export: exportRuntimeState, import: importRuntimeState };
}
...
function installSessionBridge() {
  ...
  try {
    ...
    if (!sessions?.saveFile) return;
    const originalSave = sessions.saveFile.bind(sessions);
    const originalOpen = sessions.openFile?.bind(sessions);
    sessions.saveFile = async (args = {}) => originalSave({ ...args, data: { ...(args?.data || {}), testsRuntime: exportRuntimeState() } });
    if (originalOpen) {
    ...
    console.warn("[JW Modbus Tool] Session bridge no pudo parchearse; la vista seguirá funcionando.", error);
  }
}
...
function safeSyncOverlay() { try { installSessionBridge(); if (!active()) { cleanupOverlay(); return; } const o = ensureOverlay(); if (!o.dataset.rendered) render(); } catch (error) { console.error("[JW Modbus Tool] Simple tests overlay sync failed", error); cleanupOverlay(); } }
function render() {
  if (!active()) return;
...
function execTable() { const r = st.log.length ? st.log : st.steps; return `<table><thead><tr><th>Hora</th><th>Paso</th><th>Slave</th><th>Función</th><th>Dirección</th><th>Cantidad/Valor</th><th>Resultado</th><th>Tiempo</th><th>Detalle</th></tr></thead><tbody>${r.map((x, i) => `<tr><td>${x.at || "—"}</td><td>${i + 1}</td><td>${x.slave}</td><td>${F[x.fn]}</td><td>${x.addr}</td><td>${esc(x.amount)}</td><td>${res(x.res)}</td><td>${x.ms != null ? x.ms + " ms" : "—"}</td><td>${esc(x.detail || x.values || "Pendiente de ejecución.")}</td></tr>`).join("")}</tbody></table>`; }
function scHtml() { return Object.keys(st.scenarios).map((k) => { const s = st.scenarios[k]; return `<button class="scenarioCard ${st.sc === k ? "selected" : ""}" data-a="sc" data-sc="${k}"><b class="scenarioIcon ${s.color}">${esc(s.icon)}</b><span><strong>${esc(s.name)}</strong><small>${esc(s.desc)}</small></span><i class="scenarioRadio">●</i></button>`; }).join(""); }
function managerHtml() { const keys = Object.keys(st.scenarios); const s = st.scenarios[st.sc] || st.scenarios.normal; const previewSteps = (s.steps || st.steps).slice(0, 6); return `<div class="scenarioManager"><label>Escenario activo<select data-sel="scenario">${keys.map((k) => `<option value="${k}" ${k === st.sc ? "selected" : ""}>${esc(st.scenarios[k].name)}</option>`).join("")}</select></label><div class="scenarioPreview"><b class="scenarioIcon ${s.color}">${esc(s.icon)}</b><span><strong>${esc(s.name)}</strong><small>${esc(s.desc)}</small></span></div><label>Nombre<input data-sf="name" value="${esc(s.name)}"></label><label>Descripción<textarea data-sf="desc">${esc(s.desc)}</textarea></label><div class="managerRow"><label>Ícono<input data-sf="icon" value="${esc(s.icon)}" maxlength="2"></label><label>Color<select data-sf="color"><option value="shield" ${s.color === "shield" ? "selected" : ""}>Verde</option><option value="clock" ${s.color === "clock" ? "selected" : ""}>Naranja</option><option value="warn" ${s.color === "warn" ? "selected" : ""}>Amarillo</option><option value="bad" ${s.color === "bad" ? "selected" : ""}>Rojo</option></select></label></div><section class="scenarioSteps"><h3>Pasos del escenario</h3><ol>${previewSteps.map((x) => `<li>${F[x.fn]} · S${x.slave} · ${x.addr} · ${esc(x.amount)}</li>`).join("")}</ol></section><button data-a="scenarioUsePlan">Usar plan actual como pasos</button><div class="managerActions"><button data-a="scenarioDuplicate">Duplicar</button><button class="primary" data-a="scenarioClose">Guardar cambios</button></div><p class="managerNote">Los cambios quedan listos para persistirse dentro de <b>.jwmodbus-session</b> al usar Guardar sesión.</p></div>`; }
...
async function run() {
  ...
  st.log = []; st.running = true; st.stopped = false; render();
  for (let i = 0; i < st.steps.length; i++) {
    if (st.stopped) break;
    const x = st.steps[i]; if (!x.on) continue;
    const t = performance.now(); const at = new Intl.DateTimeFormat("es-PE", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(new Date());
    try {
      const r = await exec(x, b); const ms = Math.round(performance.now() - t);
      if (!r.ok) { x.res = String(r.error || "").toLowerCase().includes("timeout") ? "Timeout" : "Error"; x.detail = r.error || "Error de comunicación."; x.ms = ms; x.at = at; }
      else { const v = r.value || {}; const vals = v.registerValues || v.values || writeEchoValues(x); x.res = v.exception ? "Excepción" : v.crcOk === false ? "CRC Error" : "Aprobado"; x.ms = v.elapsedMs ?? ms; x.values = summarizeValues(x, vals); x.detail = v.exception?.exceptionName || successDetail(x, vals); x.at = at; }
    } catch (e) { x.res = "Error"; x.ms = Math.round(performance.now() - t); x.detail = e?.message || "Error inesperado."; x.at = at; }
    st.log = st.steps.filter((z) => z.at); render();
  }
  st.running = false; persistPlanMessage(); msg(st.stopped ? "Prueba detenida por el usuario." : "Plan de pruebas ejecutado. Revisa resultados, tiempos y detalles.");
}
...
function summarizeValues(x, vals) { if (!vals?.length) return ""; return vals.slice(0, 4).map((y, j) => `${disp(x.fn, raw(x.fn, x.addr) + j, x.addr)}=${typeof y === "boolean" ? (y ? "ON" : "OFF") : y}`).join(", ") + (vals.length > 4 ? ", ..." : ""); }
function successDetail(x, vals) { if (x.fn === "fc5") return `Coil escrita en ${normalizeBool(x.amount) ? "ON" : "OFF"}.`; if (x.fn === "fc6") return `Registro escrito con valor ${Number(x.amount)}.`; if (x.fn === "fc15") return `${parseBoolList(x.amount).length} coils escritas: ${summarizeValues(x, vals)}.`; if (x.fn === "fc16") return `${parseRegList(x.amount).length} registros escritos: ${summarizeValues(x, vals)}.`; return `${amountNumber(x)} ${x.fn === "fc1" ? "coils" : x.fn === "fc2" ? "bits" : "registros"} leídos correctamente.`; }
function applySc(k) { st.sc = k; const custom = st.scenarios[k]?.steps; if (custom?.length) st.steps = custom.map(cleanStep); else { const s = sid(); if (k === "normal") st.steps = plan(s); if (k === "timeout") st.steps = [mk(247, "fc3", 40000, 1, "Timeout", 350)]; if (k === "exception") st.steps = [mk(s, "fc3", 49999, 10, "Excepción", 1000)]; if (k === "crc") st.steps = plan(s).map((x) => ({ ...x, exp: x.exp === "OK" ? "CRC/Error controlado" : x.exp })); } st.log = []; persistPlanMessage(); render(); }
...
function markDirty() { st.log = []; persistPlanMessage(); render(); }

document.addEventListener("click", (e) => { const o = overlay(); const a = e.target.closest?.("[data-a]"); if (!o || !a || !o.contains(a)) return; e.preventDefault(); const n = a.dataset.a; if (n === "run") run(); if (n === "stop") { st.stopped = true; msg("Deteniendo prueba al finalizar el paso actual..."); } if (n === "add") { st.steps.push(mk(sid(), "fc3", 40000, 1, "1 reg", 1000)); markDirty(); } if (n === "del") { st.steps.splice(+a.dataset.i, 1); markDirty(); } if (n === "clear") { st.log = []; st.steps = st.steps.map(cloneStep); render(); } if (n === "export") { navigator.clipboard?.writeText(st.log.map((x, i) => `${i + 1}. ${x.at} ${F[x.fn]} S${x.slave} ${x.res} ${x.detail}`).join("\n")); msg("Registro copiado al portapapeles."); } if (n === "save") persistPlanMessage(); if (n === "sc") applySc(a.dataset.sc); if (n === "manage") { st.manage = true; render(); } if (n === "scenarioClose") { st.manage = false; persistPlanMessage(); render(); } if (n === "scenarioUsePlan") { st.scenarios[st.sc].steps = st.steps.map(cleanStep); msg("Plan actual asignado al escenario seleccionado."); persistPlanMessage(); render(); } if (n === "scenarioDuplicate") { const base = st.scenarios[st.sc]; const id = `custom_${Date.now()}`; st.scenarios[id] = { ...JSON.parse(JSON.stringify(base)), name: `${base.name} copia`, steps: (base.steps || st.steps).map(cleanStep) }; st.sc = id; msg("Escenario duplicado. Edita su nombre, color, ícono y pasos."); persistPlanMessage(); render(); } if (n === "sim") { st.sim = st.sim === "Detenido" ? "Preparado" : "Detenido"; persistPlanMessage(); render(); } });
document.addEventListener("change", (e) => { const o = overlay(); const t = e.target; if (!o || !t || !o.contains(t)) return; if (t.dataset?.sel === "scenario") { st.sc = t.value; render(); return; } if (t.dataset?.sf) { st.scenarios[st.sc][t.dataset.sf] = t.value; persistPlanMessage(); render(); return; } const c = t.dataset?.c; const i = +t.dataset?.i; if (!c || !st.steps[i]) return; const x = st.steps[i]; const v = t.type === "checkbox" ? t.checked : t.value; if (c === "on") x.on = !!v; if (c === "slave") x.slave = Math.max(1, Math.min(247, Number.parseInt(String(v).replace(/\D/g, ""), 10) || 1)); if (c === "fn") { x.fn = v; x.addr = def(v); x.amount = defaultAmount(v); x.exp = defaultExpected(v, x.amount); } if (c === "addr") x.addr = +v; if (c === "amount") { x.amount = String(v); if (readFns.has(x.fn)) x.exp = defaultExpected(x.fn, x.amount); } if (c === "exp") x.exp = v; if (c === "to") x.to = +v; x.res = "Pendiente"; markDirty(); });
document.addEventListener("input", (e) => { const o = overlay(); const t = e.target; if (!o || !t || !o.contains(t) || !t.dataset?.sf) return; st.scenarios[st.sc][t.dataset.sf] = t.value; exposeRuntimeState(); });
...
function start() {
  try {
    ...
    window.setInterval(safeSyncOverlay, 250);
    window.addEventListener("resize", () => positionOverlay());
    new MutationObserver(safeSyncOverlay).observe(document.body, { childList: true, subtree: true });
    safeSyncOverlay();
  } catch (error) {
    console.error("[JW Modbus Tool] Simple tests runtime safe init failed", error);
  }
}
