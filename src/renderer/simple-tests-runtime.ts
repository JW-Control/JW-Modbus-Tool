...
function scHtml() {
  return Object.keys(st.scenarios).map((k) => {
...
document.addEventListener("change", (e) => {
  ...
  if (c === "fn") { x.fn = v; x.addr = def(v); x.amount = defaultAmount(v); x.exp = defaultExpected(v, x.amount); }
  if (c === "addr") x.addr = +v;
...
document.addEventListener("input", (e) => {
  const o = overlay();
  const t = e.target;
  if (!o || !t || !o.contains(t) || !t.dataset?.sf) return;
