...
export function SimpleModeApp() {
  ...
  async function readRegisters(fn: Fn, unitId: number, displayAddress: number, quantity: number, timeoutMs = timeout) {
    ...
    pushActivity({ ms: action.elapsedMs ?? Date.now() - started.getTime(), slave: unitId, device: displayNameFor(unitId), fn: fnMap[fn].short, label: fnMap[fn].label, range: `${displayAddress} / ${quantity} ${fn === "fc1" || fn === "fc2" ? "bits" : "regs"}`, qty: quantity, values: valuesPreview(fn, displayAddress, values), status });
  ...
  function showActivityInfo(row: ActivityRow) { setMessage(`${row.label}: ${row.values}. Slave ${row.slave}, ${row.range}, duración ${row.ms} ms.`); }
