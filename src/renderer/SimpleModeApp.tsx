...
const fnMap: Record<Fn, { short: string; label: string }> = {
  ...
  fc6: { short: "FC06", label: "FC06 Write Single" }
};
...
export function SimpleModeApp() {
  ...
  useEffect(() => {
    if (!selectedRegister && regs.length > 0) setSelectedRegister(regs[0]);
  }, [regs, selectedRegister]);
  ...
  function setVisiblePorts(rawPorts: any[], preferred = port) {
    ...
    if (!preferred && withMetadata.length === 0 && normalized.length > 1) {
      visible = [highestComPort(normalized)];
    }
  ...
  async function refreshPorts() {
    if (!bridge) {
      setMessage("Backend Electron no disponible.");
      return;
    }
    setBusy(true);
  ...
  async function connect() {
    ...
    if (!port) {
      setMessage("No hay puerto seleccionado. Recarga la lista de puertos.");
      return;
    }
    setBusy(true);
  ...
  function requireReady() {
    if (!bridge) {
      setMessage("Abre la app en Electron para usar el backend serial.");
      return false;
    }
    if (!connected) {
      setMessage("Conecta primero el puerto serial.");
      return false;
    }
    return true;
  }
  ...
  async function scanDevices() {
    ...
    for (const id of scanRange) {
      ...
      const result = await bridge!.modbus.readHoldingRegisters({
        unitId: id,
        startAddress: 0,
        quantity: 1,
        timeoutMs: 250
      ...
      if (result.ok) {
        ...
        pushActivity({
          ...
          device: device.name,
          fn: "FC03",
          label: "Escaneo detectó respuesta",
          range: "0 / 1 reg",
          qty: 1,
          status: "OK"
        });
    ...
    setStats((current) => ({
      ...current,
      requests: current.requests + scanRange.length,
      responses: current.responses + found.length
    }));
  ...
  async function readRegisters(fn: Fn, unitId: number, displayAddress: number, quantity: number, timeoutMs = timeout) {
    ...
    setStats((current) => ({
      ...current,
      responses: current.responses + (status === "OK" ? 1 : 0),
      errors: current.errors + (status === "OK" ? 0 : 1)
    }));
    ...
    pushActivity({
      ...
      range: `${displayAddress} / ${quantity} regs`,
      qty: quantity,
      status
    });
  ...
  async function quickRead() {
    if (activeId === null) {
      setMessage("Primero escanea y selecciona un slave activo.");
      return;
    }
    setBusy(true);
  ...
  async function runTests() {
    if (activeId === null) {
      setMessage("Primero detecta un slave activo.");
      return;
    }
    if (!requireReady()) return;
    setBusy(true);
    ...
    for (const test of next) {
      ...
      if (test.fn === "fc6") {
        const started = new Date();
        setStats((current) => ({ ...current, requests: current.requests + 1 }));
        const result = await bridge!.modbus.writeSingleRegister({
          unitId: test.slave,
          address: rawAddress(test.fn, test.address),
          value: test.amount,
  ...
  function pushActivity(row: Omit<ActivityRow, "id" | "at">) {
    setActivity((current) => [{ id: rowId++, at: new Date(), ...row }, ...current].slice(0, 30));
  }
  ...
  function pushTrafficError(fn: Fn, unitId: number, device: string, error: string, at: Date, status: Status) {
    setTraffic((current) => [{ id: rowId++, dir: "up", at, route: `PC Master → ${device}`, slave: unitId, type: "Petición", fn: fnMap[fn].short, status, summary: error }, ...current].slice(0, 80));
  }

  function displayNameFor(id: number) {
    return devices.find((device) => device.id === id)?.name ?? `Slave ID ${id}`;
  }
    ...
    <main className="shell">
      <header className="toolbar">
        ...
        <div className="toolbar-actions">
          ...
          <i />
          <Tool icon={Plug} label="Conectar" tone="ok" onClick={connect} />
          <Tool icon={Unplug} label="Desconectar" tone="danger" onClick={disconnect} />
          <i />
          <Tool icon={Search} label="Escanear" onClick={scanDevices} />
        </div>
        <div className="window-buttons"><span>{busy ? "Procesando…" : "Ayuda"}</span><button>—</button><button>□</button><button>×</button></div>
      </header>
      ...
      <div className="body">
        <aside className="sidebar">
          ...
          <div className="helper"><Info size={18} /><strong>¿Cómo funciona?</strong><p>{message}</p></div>
          <div className="license"><Dot />Licencia: Profesional<br /><small>Versión 1.3.0 (64-bit)</small></div>
        </aside>
        <section className="workspace">
          {view === "devices" && <DevicesView {...{ ports, port, setPort, refreshPorts, baud, setBaud, dataBits, setDataBits, parity, setParity, stopBits, setStopBits, timeout, setTimeoutMs, connected, devices, activeId, activeDevice, selectDevice, scanDevices, lastScan, stats, qFn, setQFn, qAddr, setQAddr, qQty, setQQty, quick, quickRead, activity }} />}
          {view === "sessions" && <SessionsView activity={activity} devices={devices} />}
          {view === "tests" && <TestsView tests={tests} runTests={runTests} busy={busy} />}
          {view === "registers" && <RegistersView {...{ activeDevice, rFn, setRFn, rAddr, setRAddr, rQty, setRQty, regs, selectedRegister, setSelectedRegister, registerRead, activity }} />}
          {view === "traffic" && <TrafficView traffic={traffic} />}
        </section>
      </div>

      <footer className="status"><Dot /><strong>{connected ? "Conectado" : "Inactivo"}</strong><span>{port || "Sin puerto"}</span><span>{baud}</span><span>{dataBits}{parity === "none" ? "N" : parity[0].toUpperCase()}{stopBits}</span><span>Slave activo ID {activeId ?? "—"}</span><span>Sesión limpia</span></footer>
    </main>
  );
}
  ...
  return <div className="devices grid">
    <Card title="1. Conectar" className="connect">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "end" }}>
        <SelectField label="Puerto" value={props.port} onChange={props.setPort} options={props.ports.map((item) => item.path)} empty="Sin puertos" />
        <button className="ghost" onClick={props.refreshPorts} title="Recargar puertos disponibles"><RefreshCw size={15} /></button>
      </div>
      ...
      <NumberField label="Timeout" value={props.timeout} onChange={props.setTimeoutMs} suffix="ms" />
      <p className="connection"><Dot />Estado de conexión <strong>{props.connected ? "Conectado" : "Desconectado"}</strong></p>
    </Card>

    <Card title="2. Dispositivos detectados" action={<button className="ghost" onClick={props.scanDevices}><RefreshCw size={15} />Recargar</button>} className="detected">
      {props.devices.length === 0 ? <Empty text="Sesión limpia: todavía no hay dispositivos detectados. Usa Recargar/Escanear." /> : <Table columns={["Dispositivo", "ID"]} rows={props.devices.map((device) => [<button style={rowButtonStyle} onClick={() => props.selectDevice(device.id)}><Dot />{device.name} — ID {device.id}{device.id === props.activeId && <b className="pill">SLAVE ACTIVO</b>}{!device.named && <b className="pill">NOMBRE PEND.</b>}</button>, device.id])} />}
      <p className="note"><Info size={16} />Los nombres personalizados se asignarán desde la sesión o desde el mapa del dispositivo.</p>
    </Card>

    <Card title="Resumen" className="summary">
      <p>Estado general de la comunicación.</p>
      <div className="kpis"><Kpi icon="↗" label="Solicitudes" value={String(props.stats.requests)} /><Kpi icon="✓" label="Respuestas" value={String(props.stats.responses)} tone="ok" /><Kpi icon="!" label="Errores" value={String(props.stats.errors)} tone="warn" /><Kpi icon="◷" label="Timeouts" value={String(props.stats.timeouts)} tone="purple" /></div>
      <dl><dt>Dispositivos encontrados</dt><dd>{props.devices.length}</dd><dt>Rango de escaneo</dt><dd>1 – 10</dd><dt>Último escaneo</dt><dd>{props.lastScan ? <><Status status="OK" /><small>{fmt(props.lastScan)}</small></> : "—"}</dd><dt>Rol</dt><dd><Monitor size={15} /> PC Master</dd></dl>
    </Card>

    <div style={{ gridColumn: "1 / 4", display: "grid", gridTemplateColumns: "0.44fr 0.56fr", gap: 14 }}>
      <Card title="3. Lectura rápida de registros" className="quickPanel">
        ...
        <div className="quickbar"><SelectField label="Función" value={props.qFn} onChange={(value) => props.setQFn(value as Fn)} options={["fc3", "fc4", "fc1", "fc2"]} labels={fnLabels()} /><NumberField label="Dirección inicial" value={props.qAddr} onChange={props.setQAddr} /><NumberField label="Cantidad" value={props.qQty} onChange={props.setQQty} /><button className="primary" onClick={props.quickRead} disabled={!props.activeDevice}><Database size={15} />Leer</button></div>
        {props.quick.length === 0 ? <Empty text="Aún no hay lecturas. La tabla se llenará al presionar Leer." /> : <Table columns={["Dirección", "Nombre", "Valor", "Estado"]} rows={props.quick.map((row) => [row.address, withUnit(row), row.value, <Status status={row.status} />])} />}
      ...
      <Card title="Actividad reciente" className="recentPanel">
        {props.activity.length === 0 ? <Empty text="Sin actividad todavía. Aquí aparecerán escaneos y lecturas reales." /> : <Table columns={["Fecha/hora", "Duración", "Slave ID", "Función", "Dirección / cantidad", "Resultado"]} rows={props.activity.slice(0, 6).map((row) => [fmt(row.at), `${row.ms} ms`, row.slave, row.fn, row.range, <Status status={row.status} />])} />}
      </Card>
    </div>
  </div>;
}

function SessionsView({ activity, devices }: { activity: ActivityRow[]; devices: Device[] }) {
...
function TestsView({ tests, runTests, busy }: { tests: TestRow[]; runTests: () => void; busy: boolean }) {
...
function normalizePorts(rawPorts: any[]): PortOption[] {
  const unique = new Map<string, PortOption>();
  for (const raw of rawPorts) {
    if (!raw?.path) continue;
    const hasMetadata = Boolean(raw.vendorId || raw.productId || raw.manufacturer || raw.serialNumber || raw.pnpId);
    const label = raw.displayName || raw.friendlyName || raw.path;
    unique.set(raw.path, { path: raw.path, label, hasMetadata });
  }
  return [...unique.values()].sort((left, right) => portNumber(left.path) - portNumber(right.path));
}
...
function buildRegisterRows(fn: Fn, start: number, values: Array<number | boolean>): RegisterRow[] { const rawStart = rawAddress(fn, start); return values.map((value, index) => { const raw = rawStart + index; const meta = registerMeta[raw] ?? { name: `Reg_${displayAddress(fn, raw)}`, type: typeof value === "boolean" ? "bool" : "uint16", access: fn === "fc1" || fn === "fc3" ? "R/W" : "R" }; return { raw, address: String(displayAddress(fn, raw)), name: meta.name, value: typeof value === "boolean" ? (value ? "ON" : "OFF") : String(value), type: meta.type, access: meta.access, status: "OK" }; }); }
