...
export function SimpleModeApp() {
  ...
  const currentSignature = useMemo(() => signatureFromState({ sessionName, port, baud, dataBits, parity, stopBits, timeout, devices, activeId, stats, quick, regs, activity, traffic, tests, notes }), [sessionName, port, baud, dataBits, parity, stopBits, timeout, devices, activeId, stats, quick, regs, activity, traffic, tests, notes]);
  ...
  function pushTrafficAction(action: { timestamp?: string; unitId: number; elapsedMs?: number; registerValues?: unknown[]; values?: unknown[]; exception?: { exceptionName?: string } }, fn: Fn, device: string, address: number, quantity: number, status: Status) {
...
function SessionsView(props: { sessionName: string; setSessionName: (value: string) => void; recentSessions: RecentSession[]; onNew: () => void; onSave: () => void; onSaveAs: () => void; onOpen: (filePath?: string) => void; activity: ActivityRow[]; devices: Device[]; stats: Stats; traffic: TrafficRow[]; tests: TestRow[]; connected: boolean; port: string; baud: number; dataBits: number; parity: string; stopBits: number; activeId: number | null; sessionId: string | null; sessionFilePath: string | null; sessionState: SessionState; notes: string; setNotes: (value: string) => void }) {
...
function responseSummary(action: { exception?: { exceptionName?: string }; registerValues?: unknown[]; values?: unknown[] }) { if (action.exception) return action.exception.exceptionName ?? "Excepción Modbus"; if (action.registerValues) return `${action.registerValues.length} registros`; if (action.values) return `${action.values.length} bits`; return "Escritura OK"; }
